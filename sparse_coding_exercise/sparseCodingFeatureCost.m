...
%                   otherwise.
    ...
    least_squares_grad = 2 * weightMatrix' * (weightMatrix * featureMatrix - patches);
    sparsity_control_grad = lambda * groupMatrix' * (groupMatrix * (featureMatrix .* featureMatrix) + epsilon) .^ -0.5 .* featureMatrix;
	grad = least_squares_grad + sparsity_control_grad;
    grad = grad(:);
